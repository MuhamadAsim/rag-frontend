...
const ChatHistory = ({ chats, currentChat, onSelectChat, onDeleteChat }: ChatHistoryProps) => {
  ...
  return (
    <Card className="h-full flex flex-col gradient-card shadow-soft"> {/* ✅ flex column */}
      {/* Header */}
      ...
      <ScrollArea className="flex-1 overflow-auto"> {/* ✅ fills remaining height */}
          ...
          <div className="p-2 space-y-2">
            {filteredChats
              ...
              .map((chat) => (
                <div
                  key={chat.id}
                  className={`group p-3 rounded-lg cursor-pointer transition-smooth ${currentChat?.id === chat.id
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <h4
                        ...
                        title={chat.title} // 👈 full title on hover
                      >
                        {chat.title
                          ? chat.title.split(" ").slice(0, 2).join(" ") + (chat.title.split(" ").length > 2 ? " ..." : "")
                          : "Untitled Chat"}
                      </h4>
